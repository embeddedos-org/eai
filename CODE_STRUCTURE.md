...
| `preference.set`     | key, value            | —         | preference:write | Store a user preference              |
| `preference.get`     | key                   | —         | preference:read  | Retrieve a user preference           |
| `feedback.record`    | score                 | context   | feedback:write   | Record inference quality feedback    |
...
eai_tools_register_builtins(&reg);   // registers all 7
```c
eai_tool_registry_t reg;
eai_tool_registry_init(&reg);
eai_tools_register_builtins(&reg);   // registers all 7
...
## Adaptive / Personalized AI Module

### adaptive.h / adaptive.c (Common)

The core adaptive types and data stores used by both EAI-Min and EAI-Framework:

- **`eai_preference_store_t`** — Fixed 256-entry typed key-value store. Entries have weights (0.0–1.0) and per-entry decay rates. When full, new entries evict the lowest-weight existing entry.
- **`eai_feedback_buffer_t`** — Ring buffer of 512 feedback entries. Each entry records a quality score (-1.0 to +1.0) for an input/output pair.
- **`eai_training_buffer_t`** — Ring buffer of 256 training samples generated from positive feedback.
- **`eai_adaptive_config_t`** — Configuration struct for learning rate, LoRA rank, decay timing, memory budgets.

Persistence format: `key|value|type|weight|decay\n` (pipe-delimited text, matching `memory_lite.c` patterns).

### adaptive.h / adaptive.c (Framework)

The framework-level adaptive engine (`eai_fw_adaptive_t`) orchestrates the full learning pipeline:

```
Feedback → Feedback Buffer → Training Sample → Training Buffer → train_step() → LoRA Adapter
```

Resource gating checks RAM and CPU temperature before each training cycle. Policy knobs control step limits and data retention.

### Training Flow

```
User Feedback (score, context)
        │
        ▼
┌─────────────────┐
│ Feedback Buffer  │ ← ring buffer, 512 entries
│ (eai_feedback_t) │
└────────┬────────┘
         │ positive feedback (score > 0.5)
         ▼
┌─────────────────┐
│ Training Buffer  │ ← ring buffer, 256 samples
│ (eai_training_   │
│  sample_t)       │
└────────┬────────┘
         │ batch extraction
         ▼
┌─────────────────┐
│ Runtime vtable   │ ← train_step() op
│ (LoRA training)  │
└────────┬────────┘
         │ weight update
         ▼
┌─────────────────┐
│ LoRA Adapter     │ ← 5-100 MB file
│ (checkpoint)     │
└─────────────────┘
```
