...
static eai_status_t exec_step(eai_fw_orchestrator_t *orch, const eai_workflow_step_t *step)
{
    ...
    case EAI_STEP_LEARN: {
        if (!orch->adaptive) {
            EAI_LOG_WARN(LOG_MOD, "learn step skipped — no adaptive engine");
            return EAI_OK;
        }
        return eai_fw_adaptive_run_training_cycle((eai_fw_adaptive_t *)orch->adaptive);
    }

    case EAI_STEP_FEEDBACK: {
        if (!orch->adaptive) return EAI_OK;
        const char *score_str = NULL;
        const char *context = NULL;
        for (int i = 0; i < step->param_count; i++) {
            if (strcmp(step->params[i].key, "score") == 0) score_str = step->params[i].value;
            if (strcmp(step->params[i].key, "context") == 0) context = step->params[i].value;
        }
        float score = score_str ? (float)atof(score_str) : 0.0f;
        return eai_fw_adaptive_record_feedback((eai_fw_adaptive_t *)orch->adaptive,
                                                score, context, NULL, NULL);
    }

    case EAI_STEP_PREFERENCE: {
        if (!orch->adaptive) return EAI_OK;
        eai_fw_adaptive_t *a = (eai_fw_adaptive_t *)orch->adaptive;
        const char *key = NULL, *value = NULL;
        for (int i = 0; i < step->param_count; i++) {
            if (strcmp(step->params[i].key, "key") == 0)   key = step->params[i].value;
            if (strcmp(step->params[i].key, "value") == 0) value = step->params[i].value;
        }
        if (key && value && a->prefs) {
            return eai_pref_set(a->prefs, key, value, EAI_PREF_STRING, 0.8f);
        }
        return EAI_OK;
    }
