...
typedef struct {
    ...
    /* Training metadata (adaptive models) */
    uint32_t        train_ram_mb;     /* RAM needed for LoRA fine-tuning */
    bool            lora_supported;   /* whether LoRA fine-tuning is feasible */
    uint32_t        lora_adapter_mb;  /* expected LoRA adapter disk size */
} eai_model_info_t;
