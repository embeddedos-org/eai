"""eAI - production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # INTRO
        title = Text("eAI", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Embedded AI Layer", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        techs = "C, Python, TensorFlow Lite, CMSIS-NN".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # FEATURES
        features = [
            ("01", "On-Device Inference", "Run neural networks directly on microcontrollers with under 256KB RAM", DUR["f1"]),
            ("02", "Model Quantization", "Automatic INT8 and binary quantization reduces model size by 4 to 32x", DUR["f2"]),
            ("03", "Hardware Acceleration", "Leverages CMSIS-NN, NPU, and DSP instructions for real-time performance", DUR["f3"]),
        ]
        ...
        components = ["Model Loader", "Quantizer", "Runtime", "HW Accel", "Inference"]
        boxes = VGroup()
        ...
        # CTA
        cta_name = Text("eAI", font_size=72, color=WHITE, weight=BOLD)
        cta_line = Line(LEFT*2, RIGHT*2, color=ACCENT, stroke_width=3)
        cta_line.next_to(cta_name, DOWN, buff=0.3)
        cta_url = Text("github.com/embeddedos-org/eAI", font_size=22, color=ManimColor(ACCENT))
