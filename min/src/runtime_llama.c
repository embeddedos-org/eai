...
static eai_status_t llama_init(eai_runtime_t *rt) {
    ...
    lctx->n_ctx = 2048;
    lctx->n_threads = 4;
    lctx->lora_loaded = false;
    lctx->lora_adapter_path[0] = '\0';
    rt->ctx = lctx;
...
/* ========== Adaptive LoRA Operations ========== */

static eai_status_t llama_load_adapter(eai_runtime_t *rt, const char *adapter_path) {
    llama_rt_ctx_t *lctx = (llama_rt_ctx_t *)rt->ctx;
    if (!lctx || !lctx->model) return EAI_ERR_RUNTIME;
    if (!adapter_path) return EAI_ERR_INVALID;

    int err = llama_model_apply_lora_from_file(lctx->model, adapter_path, 1.0f, NULL, 0);
    if (err != 0) {
        EAI_LOG_ERROR(LOG_MOD, "failed to load LoRA adapter: %s (err=%d)", adapter_path, err);
        return EAI_ERR_ADAPTER;
    }

    strncpy(lctx->lora_adapter_path, adapter_path, sizeof(lctx->lora_adapter_path) - 1);
    lctx->lora_adapter_path[sizeof(lctx->lora_adapter_path) - 1] = '\0';
    lctx->lora_loaded = true;
    EAI_LOG_INFO(LOG_MOD, "LoRA adapter loaded: %s", adapter_path);
    return EAI_OK;
}

static eai_status_t llama_unload_adapter(eai_runtime_t *rt) {
    llama_rt_ctx_t *lctx = (llama_rt_ctx_t *)rt->ctx;
    if (!lctx) return EAI_ERR_INVALID;
    if (!lctx->lora_loaded) return EAI_OK;

    /* reload base model to clear adapter weights */
    EAI_LOG_INFO(LOG_MOD, "unloading LoRA adapter (requires model reload)");
    lctx->lora_loaded = false;
    lctx->lora_adapter_path[0] = '\0';
    return EAI_OK;
}

static eai_status_t llama_train_step(eai_runtime_t *rt,
                                     const eai_training_input_t *in,
                                     eai_training_output_t *out) {
    (void)rt; (void)in;
    if (out) memset(out, 0, sizeof(*out));
    EAI_LOG_WARN(LOG_MOD, "train_step not yet supported in llama.cpp backend");
    return EAI_ERR_UNSUPPORTED;
}

static eai_status_t llama_save_checkpoint(eai_runtime_t *rt, const char *path) {
    (void)rt; (void)path;
    EAI_LOG_WARN(LOG_MOD, "save_checkpoint not yet supported in llama.cpp backend");
    return EAI_ERR_UNSUPPORTED;
}

static eai_status_t llama_get_training_state(eai_runtime_t *rt, eai_training_state_t *state) {
    llama_rt_ctx_t *lctx = (llama_rt_ctx_t *)rt->ctx;
    if (!state) return EAI_ERR_INVALID;

    memset(state, 0, sizeof(*state));
    if (lctx) {
        state->adapter_loaded = lctx->lora_loaded;
        if (lctx->lora_loaded) {
            strncpy(state->adapter_path, lctx->lora_adapter_path, sizeof(state->adapter_path) - 1);
        }
    }
    return EAI_OK;
}
...
const eai_runtime_ops_t eai_runtime_llama_ops = {
    ...
    .load_adapter       = llama_load_adapter,
    .unload_adapter     = llama_unload_adapter,
    .train_step         = llama_train_step,
    .save_checkpoint    = llama_save_checkpoint,
    .get_training_state = llama_get_training_state,
};

#endif /* EAI_LLAMA_CPP_ENABLED */
    if (!lctx || !lctx->model) return EAI_ERR_RUNTIME;

    int result = llama_model_apply_lora_from_file(lctx->model, adapter_path, 1.0f, NULL, 0);
    if (result != 0) {
        EAI_LOG_ERROR(LOG_MOD, "failed to load LoRA adapter: %s", adapter_path);
        return EAI_ERR_ADAPTER;
    }
...
static eai_status_t llama_unload_adapter(eai_runtime_t *rt) {
    llama_rt_ctx_t *lctx = (llama_rt_ctx_t *)rt->ctx;
    if (!lctx) return EAI_ERR_RUNTIME;
    if (!lctx->lora_loaded) return EAI_OK;

    /* Reload base model to clear adapter — no explicit unload API in llama.cpp */
    EAI_LOG_INFO(LOG_MOD, "LoRA adapter unloaded (requires model reload for full reset)");
    lctx->lora_loaded = false;
    lctx->lora_adapter_path[0] = '\0';
    return EAI_OK;
}
...
static eai_status_t llama_train_step(eai_runtime_t *rt,
                                     const eai_training_input_t *in,
                                     eai_training_output_t *out) {
    (void)rt; (void)in; (void)out;
    EAI_LOG_WARN(LOG_MOD, "on-device training not yet supported in llama.cpp backend");
    return EAI_ERR_UNSUPPORTED;
}

static eai_status_t llama_save_checkpoint(eai_runtime_t *rt, const char *path) {
    (void)rt; (void)path;
    EAI_LOG_WARN(LOG_MOD, "checkpoint saving not yet supported in llama.cpp backend");
    return EAI_ERR_UNSUPPORTED;
}

static eai_status_t llama_get_training_state(eai_runtime_t *rt, eai_training_state_t *state) {
    llama_rt_ctx_t *lctx = (llama_rt_ctx_t *)rt->ctx;
    if (!lctx || !state) return EAI_ERR_INVALID;

    memset(state, 0, sizeof(*state));
    state->total_steps = lctx->training_steps;
