...
eai_status_t eai_min_agent_set_adaptive(eai_min_agent_t *agent,
...
static void append_preferences_to_prompt(const eai_min_agent_t *agent, char *prompt, size_t prompt_size)
{
    if (!agent->preferences || agent->preferences->count == 0) return;

    size_t plen = strlen(prompt);
    int written = snprintf(prompt + plen, prompt_size - plen, "\n[User Preferences]\n");
    if (written <= 0) return;
    plen += (size_t)written;

    /* sort by weight descending — simple selection of top 10 */
    int indices[EAI_PREF_MAX_ENTRIES];
    int count = agent->preferences->count;
    for (int i = 0; i < count; i++) indices[i] = i;

    for (int i = 0; i < count - 1 && i < 10; i++) {
        int max_idx = i;
        for (int j = i + 1; j < count; j++) {
            if (agent->preferences->entries[indices[j]].weight >
                agent->preferences->entries[indices[max_idx]].weight)
                max_idx = j;
        }
        if (max_idx != i) {
            int tmp = indices[i];
    ...
    int max_prefs = count < 10 ? count : 10;
    for (int i = 0; i < max_prefs && plen < prompt_size - 64; i++) {
        const eai_preference_t *p = &agent->preferences->entries[indices[i]];
        written = snprintf(prompt + plen, prompt_size - plen,
                           "- %s: %s (w=%.2f)\n", p->key, p->value, p->weight);
        if (written > 0) plen += (size_t)written;
    }
}
...
static eai_status_t agent_think(eai_min_agent_t *agent, const char *goal)
{
    ...
    append_preferences_to_prompt(agent, prompt, sizeof(prompt));

    /* append feedback summary */
    if (agent->feedback && agent->feedback->count > 0) {
        float avg = eai_feedback_avg_score(agent->feedback, 10);
        size_t plen = strlen(prompt);
        snprintf(prompt + plen, sizeof(prompt) - plen,
                 "\n[Feedback] avg_score=%.2f (last %d entries)\n",
                 avg, agent->feedback->count < 10 ? agent->feedback->count : 10);
    }
